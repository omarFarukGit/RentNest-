...
// =============================================
const createLandlordProperties = async (landlordId, payload) => {
    ...
    if (!categoryId && categoryName) {
        ...
        if (!category) {
            throw new Error(`Category "${categoryName}" not found. Please create the category first.`);
        }
...
// =============================================
const updateProperty = async (propertyId, userId, userRole, payload) => {
    ...
    const { categoryId, categoryName, title, description, images, price, location, bedrooms, bathrooms, size, amenities, availability, } = payload;
    // ===== ক্যাটেগরি হ্যান্ডেল =====
    let finalCategoryId = categoryId;
    if (!categoryId && categoryName) {
        const category = await prisma.category.findFirst({
...
// =============================================
const getLandlordProperties = async (landlordId, query) => {
...
// =============================================
const togglePropertyAvailability = async (propertyId, landlordId) => {
    const existingProperty = await prisma.property.findUnique({
        where: { id: propertyId },
    });
    if (!existingProperty) {
