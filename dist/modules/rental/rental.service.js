...
const getMyRentalRequests = async (tenantId, query) => {
    ...
    const { status, sortBy = "createdAt", sortOrder = "desc", limit = 10, page = 1, } = query;
...
// =============================================
const getLandlordRentalRequests = async (landlordId, query) => {
    // ===== ভ্যালিডেশন =====
    if (!landlordId) {
        throw new Error("User ID is required");
    }
    const { status, propertyId, sortBy = "createdAt", sortOrder = "desc", limit = 10, page = 1, } = query;
    const skip = (parseInt(page) - 1) * parseInt(limit);
    const where = {
        landlordId,
    };
...
// =============================================
const getSingleRentalRequest = async (rentalId, userId, userRole) => {
    // ===== ভ্যালিডেশন =====
    if (!rentalId) {
...
// =============================================
const updateRentalStatus = async (rentalId, landlordId, payload) => {
    // ===== পেলোড চেক =====
    if (!payload) {
        throw new Error("Request body is required");
    }
...
// =============================================
const cancelRentalRequest = async (rentalId, tenantId) => {
    // ===== ভ্যালিডেশন =====
    if (!rentalId) {
        throw new Error("Rental request ID is required");
    }
...
// =============================================
const getRentalStats = async (userId, userRole) => {
    // ===== ভ্যালিডেশন =====
    if (!userId) {
        throw new Error("User ID is required");
    }
