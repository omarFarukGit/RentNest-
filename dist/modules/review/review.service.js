...
const getPropertyReviews = async (propertyId, query) => {
    ...
    const { rating, sortBy = "createdAt", sortOrder = "desc", limit = 10, page = 1, } = query;
    // ===== প্রপার্টি আছে কিনা চেক =====
    const property = await prisma.property.findUnique({
        where: { id: propertyId },
        select: {
...
const getUserReviews = async (userId, query) => {
    ...
    const { sortBy = "createdAt", sortOrder = "desc", limit = 10, page = 1, } = query;
    const skip = (parseInt(page) - 1) * parseInt(limit);
    const [reviews, total] = await Promise.all([
        prisma.review.findMany({
...
// =============================================
const getPropertyRatingStats = async (propertyId) => {
    // ===== ভ্যালিডেশন =====
    if (!propertyId) {
        throw new Error("Property ID is required");
    }
